...
export async function getMe(memberId: string, db: PrismaClient) {
  const member = await db.member.findUnique({
    ...
    if (activeSubscription) {
      const lastFailedPaymentDate = activeSubscription.payments.length > 0 
        ? activeSubscription.payments[0].createdAt 
        : null;
      ...
      const now = new Date();
      const diffMonths = (now.getFullYear() - streakStartDate.getFullYear()) * 12 + (now.getMonth() - streakStartDate.getMonth());
