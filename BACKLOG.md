...
- [ ] `pnpm build` runs across all packages without error (empty output is fine)
- [ ] `pnpm lint` and `pnpm typecheck` run without error
- [ ] Folder structure matches AGENTS.md §2 exactly
...
- [ ] `pnpm --filter web build` produces a valid build
- [ ] `GET /api/health` returns `200 { status: "ok" }`
- [ ] All four route groups exist with placeholder pages
...
**Acceptance criteria:**
- [ ] `getUploadUrl(...)` returns a URL containing the R2 endpoint and correct key
- [ ] Key generated by `buildStorageKey("transparency", "uuid-123", "doc.pdf")` matches pattern `transparency/uuid-123/{timestamp}-doc.pdf`
- [ ] `deleteObject` calls `DeleteObjectCommand` with correct bucket and key
